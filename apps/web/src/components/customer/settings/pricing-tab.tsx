...
export function PricingTab() {
	...
	const handleSave = async () => {
		try {
			...
			if (Number.isNaN(markupNum) || markupNum < 0) {
				toast.error('Markup percentage must be a valid number 0 or greater');
				return;
			}

			if (Number.isNaN(vatNum) || vatNum < 0 || vatNum > 1) {
				toast.error('VAT rate must be between 0 and 100%');
				return;
			}

			if (Number.isNaN(depositNum) || depositNum < 0 || depositNum > 100) {
				toast.error('Deposit percentage must be between 0 and 100%');
				return;
			}

			const validityNum = parseInt(quoteValidityDays, 10);
			if (Number.isNaN(validityNum) || validityNum < 1 || validityNum > 365) {
	...
	const validityDays = parseInt(quoteValidityDays, 10) || 30;
	...
	if (error) {
		return <div className="text-destructive">Error loading pricing settings: {error.message}</div>;
	}
				...
				<CardContent>
					<FieldGroup>
						...
						<Field>
							...
							<FieldDescription>
								The default markup percentage applied to supplier costs. 100% markup means the
								retail price is 2x the supplier cost.
							</FieldDescription>
							<div className="flex items-center border border-input rounded-md w-32 focus-within:border-ring focus-within:ring-ring/50 focus-within:ring-[3px]">
								...
								/>
								<span className="text-muted-foreground text-sm px-3 border-l border-input bg-muted/50 h-9 flex items-center shrink-0">
									%
								</span>
							</div>
							<div className="mt-3 p-3 bg-muted/50 rounded-lg">
								<p className="text-sm text-muted-foreground">
									<strong>Example:</strong> A supplier cost of {'\u00A3'}
									{exampleCost.toFixed(2)} with {markupNum}% markup = {'\u00A3'}
									{exampleRetail.toFixed(2)} retail price (multiplier: {multiplier.toFixed(2)}x)
								</p>
							</div>
						</Field>
						...
						<Field>
							...
							<div className="mt-3 p-3 bg-muted/50 rounded-lg">
								<p className="text-sm text-muted-foreground">
									...
									{exampleVatBase.toLocaleString('en-GB', { minimumFractionDigits: 2 })} + {vatNum}%
									VAT = {'\u00A3'}
									{exampleVatTotal.toLocaleString('en-GB', { minimumFractionDigits: 2 })} total
								</p>
							</div>
						</Field>
						...
						<Field>
							...
							<FieldDescription>
								The default deposit percentage for new jobs. When a quote is accepted and a job is
								created, this percentage of the total will be set as the initial deposit payment.
							</FieldDescription>
							<div className="flex items-center border border-input rounded-md w-32 focus-within:border-ring focus-within:ring-ring/50 focus-within:ring-[3px]">
								<Input
									id="depositPercent"
							...
							<div className="mt-3 p-3 bg-muted/50 rounded-lg">
								<p className="text-sm text-muted-foreground">
									<strong>Example:</strong> For a {'\u00A3'}1,000 job with {defaultDepositPercent}%
									deposit — Deposit: {'\u00A3'}
									{((1000 * parseFloat(defaultDepositPercent || '0')) / 100).toFixed(2)}, Balance:{' '}
									{'\u00A3'}
									{(1000 * (1 - parseFloat(defaultDepositPercent || '0') / 100)).toFixed(2)}
								</p>
							</div>
						</Field>
						...
						<Field>
							...
							<FieldDescription>
								How many days a quote remains valid by default. This will be used to automatically
								set the "Valid Until" date when creating new quotes.
							</FieldDescription>
							...
							<div className="mt-3 p-3 bg-muted/50 rounded-lg">
								<p className="text-sm text-muted-foreground">
									<strong>Example:</strong> A quote created today would expire on{' '}
									{formattedExpiryDate}
								</p>
							</div>
						</Field>
