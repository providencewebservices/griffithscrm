...
import { type EntityEmailThread, useEntityEmailThreadsQuery } from '@/hooks/use-inbox';
	...
	return (
		...
		>
			<div className="relative mt-0.5">
				{thread.isUnread && <div className="h-2 w-2 rounded-full bg-blue-500" />}
				{!thread.isUnread && <div className="h-2 w-2" />}
			</div>
			<div className="flex-1 min-w-0">
				<div className="flex items-center justify-between gap-2 mb-0.5">
					<span
						className={`text-sm truncate ${thread.isUnread ? 'font-semibold' : 'text-muted-foreground'}`}
					>
						{senderName}
					</span>
				...
				<p
					className={`text-sm truncate ${thread.isUnread ? 'font-medium' : 'text-muted-foreground'}`}
				>
			...
			<CardHeader>
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>Email conversations matched by email address</CardDescription>
					</div>
					{total > 0 && (
						<Link to={`/app/inbox?contactEntityType=${entityType}&contactEntityId=${entityId}`}>
				...
				{isLoading ? (
					...
					<div className="flex flex-col items-center justify-center py-8 text-center">
						...
						<p className="text-xs text-muted-foreground mt-1">
							Threads are automatically matched when email addresses in messages match this
							contact's email
						</p>
					</div>
				) : (
